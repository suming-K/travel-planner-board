...
import { dbToggleChecklistItem } from '@/lib/supabase/writes'
...
export function ChecklistPage() {
  ...
  const items_raw    = useAppStore(s => s.checklistItems ?? [])
  const { user }   = useAuth()
          ...
          {sorted.map((item, idx) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => {
                    toggle(item.id, user?.id ?? 'anonymous')
                    const current = items_raw.find(i => i.id === item.id)
                    if (current) {
                      void dbToggleChecklistItem(
                        item.id,
                        !current.isChecked,
                        !current.isChecked ? (user?.id ?? null) : null
                      )
                    }
                  }}
