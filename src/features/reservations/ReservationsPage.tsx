...
import { dbUpdateReservationStatus } from '@/lib/supabase/writes'
...
export function ReservationsPage() {
  ...
  return (
    <div className="bg-[#FAFAF8] min-h-screen flex flex-col">
      ...
      {sheetId && active.find(r => r.id === sheetId) && (
        <StatusSheet
          reservation={active.find(r => r.id === sheetId) as NonNullable<typeof active[0]>}
          onClose={() => setSheetId(null)}
          onUpdate={(status, code) => {
            updateStatus(sheetId, status, code)
            void dbUpdateReservationStatus(sheetId, status, code ?? null)
            setSheetId(null)
          }}
        />
      )}
