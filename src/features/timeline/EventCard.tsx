...
export function EventCard({ event, place, reservation, isLast = false, onBook, onEdit }: EventCardProps) {
  ...
  return (
    <div className={`flex gap-[10px] px-[14px] py-[9px] items-start ${!isLast ? 'border-b border-[#EDEAE4]' : ''}`}>
      ...
      <div className="flex items-center gap-[4px] flex-shrink-0 mt-[1px]">
        ...
        {onEdit && (
          <button
            className="border-none bg-transparent cursor-pointer p-0 text-[#C0BAB2] hover:text-[#5C5550] transition-colors"
            onClick={onEdit}
