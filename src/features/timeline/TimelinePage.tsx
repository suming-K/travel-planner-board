...
export function TimelinePage({ tripId }: TimelinePageProps) {
  const selectedDate    = useAppStore(s => s.selectedDate)
  const setSelectedDate = useAppStore(s => s.setSelectedDate)
  const getEventDates   = useAppStore(s => s.getEventDates)
  const getEventsByDate = useAppStore(s => s.getEventsByDate)
  const getDayMeta      = useAppStore(s => s.getDayMeta)
  ...
  const meta   = getDayMeta(selectedDate)
  const today  = getTodayIso()
  const memory = memories.find(m => m.memoryDate === selectedDate)

  const cityId = events[0]?.cityId
      ...
      <main className="flex-1 px-4 pt-3 pb-6 flex flex-col gap-[9px]">
        <div className="flex justify-between items-baseline">
          <p className="text-[10px] text-[#B0AAA3]">
            {formatDateShort(selectedDate)}{city ? ` · ${city}` : ''}
          </p>
          {meta?.sunset && (
        ...
        {events.length === 0 ? (
          ...
          // SortableEventList wraps EventCard with dnd-kit — no EventCard props changed
          <SortableEventList
            events={events}
            places={places}
