...
interface AppState {
  ...
  updateEvent: (id: string, patch: Partial<Pick<TripEvent, 'title' | 'description' | 'startTime' | 'category' | 'priority'>>) => void
...
export const useAppStore = create<AppState>()(
  persist(
...

    {
      ...
      // createJSONStorage wraps noop → produces PersistStorage<AppState> that persist() needs
      // noop guards window so it's safe during SSR
